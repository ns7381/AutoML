 ...
 * Build: http://modernizr.com/download/#-fontface-backgroundsize-borderimage-borderradius-boxshadow-flexbox-flexboxlegacy-hsla-multiplebgs-opacity-rgba-textshadow-cssanimations-csscolumns-generatedcontent-cssgradients-cssreflections-csstransforms-csstransforms3d-csstransitions-applicationcache-canvas-canvastext-draganddrop-hashchange-history-audio-video-indexeddb-input-inputtypes-localstorage-postmessage-sessionstorage-websockets-websqldatabase-webworkers-geolocation-inlinesvg-smil-svg-svgclippaths-touch-webgl-shiv-mq-cssclasses-addtest-prefixed-teststyles-testprop-testallprops-hasevent-prefixes-domprefixes-css_boxsizing-css_calc-css_displayrunin-css_displaytable-css_lastchild
 */
;
...
/*!
 * Updated by jinzhk on 2016/11/08
 * Add Feature: urlArgs 配置项添加对 Object 和 Function 类型值的支持
 * Updated by jinzhk on 2016/12/13
 * Fixed: 修复了脚本文件存在语法错误时无法触发 error 回调处理的问题
 */
...
(function (global) {
    ...
    // fixed script error does not occured
    if (global.attachEvent
        && !(global.attachEvent.toString && global.attachEvent.toString().indexOf('[native code') < 0)
        && !isOpera
    ) {
        global.attachEvent('onerror', onScriptGlobalError);
    } else if (global.addEventListener) {
        global.addEventListener('error', onScriptGlobalError, false);
    }

    function onScriptGlobalError(evt) {
        var _script;
        eachReverse(scripts(), function(script) {
            if (script.src == evt.filename) {
                _script = script;
                return true;
            }
        });
        if (ostring.call(_script) == "[object HTMLScriptElement]" && _script.getAttribute('data-onerror') == null) {
            _script.setAttribute('data-onerror', true);
            try {
                if (_script.dispatchEvent) {
                    _script.dispatchEvent(initEvent(evt));
                } else if (_script.fireEvent) {
                    _script.fireEvent('onerror', initEvent(evt));
                }
            } catch (e) {}
        }
        function initEvent(evt) {
            ...
            try {
                ...
                _evt.initEvent('error', false, true);
            } catch (e) {
                try {
    ...
    function newContext(contextName) {
        ...
        context = {
            ...
            nameToUrl: function (moduleName, ext, skipExt) {
                ...
                if (typeof urlArgs === "object" && urlArgs != null) {
                    ...
                    for (var key in urlArgs) {
                        _urlArgs.push(key + '=' + (urlArgs[key] == null ? '' : urlArgs[key]));
                    }
                    urlArgs = _urlArgs.join('&');
                }
            ...
            onScriptLoad: function (evt) {
                ...
                //all old browsers will be supported, but this one was easy enough
                //to support and still makes sense.
                var targetEl = evt.currentTarget || evt.srcElement;
                if (targetEl.getAttribute && targetEl.getAttribute('data-onerror')) return;
                if (evt.type === 'load' ||
                        (readyRegExp.test(targetEl.readyState))) {
            ...
            onScriptError: function (evt) {
                var targetEl = evt.currentTarget || evt.srcElement;
                if (targetEl && targetEl.setAttribute) {
                    targetEl.setAttribute('data-onerror', false);
                }
                var data = getScriptData(evt);
...
/*!
 * Updated by fengyongqing on 2016/03/14
 * Add Feature: 添加了下拉框下点击某个元素不触发关闭下拉框事件的功能（含有或父元素含有[data-hide="false"]不关闭下拉框）
 */
...
+function ($) {
    ...
    function clearMenus(e) {
        ...
        $(toggle).each(function () {
            ...
            var isChild = e ? $.contains($parent.get(0), e.target) : false,
            	$noHides = $parent.is(noHide) ? $parent : $parent.find(noHide),
            	bNoHide = false;
            if (isChild) {
            	$noHides.each(function(i, element) {
...
(function($) {
  ...
  function initSettings(input, settings) {
    ...
    for(var optName in _DEFAULTS) {
      ...
      val = (!input || typeof val !== 'undefined') ? val : getDataAttrib(input[0], optName);
...
(function(window, $) {
    ...
    var BaseUrls = {
        artTemplate: YFjs.baseMd+'/artTemplate/'+Versions.artTemplate,
        codemirror: YFjs.baseMd+'/codemirror/'+Versions.codemirror,
        cryptoJS: YFjs.baseMd+'/crypto/'+Versions.cryptoJS,
        history: YFjs.baseMd+'/history/'+Versions.history,
    ...
    require.config({
        ...
        paths: {
            ...
            template: BaseUrls.artTemplate+'/template',
            'template-native': BaseUrls.artTemplate+'/template-native',
            ...
            history: BaseUrls.history+'/native.history',
            json: BaseUrls.json+'/json3',
            d3: BaseUrls.d3+'/d3.min',
